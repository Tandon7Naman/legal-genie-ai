...
const ECourtsPage = () => {
  ...
  const analysisKeyFor = (rec: any) => rec?.filename || rec?.orderUrl;

  const renderAnalysisButton = (rec: any) => {
    const key = analysisKeyFor(rec);
    if (!key) return null;
    const a = orderAnalysis[key];
    return (
      <Button
        size="sm"
        variant="ghost"
        className="h-7 text-xs px-2"
        onClick={() => streamOrderAnalysis(rec)}
        disabled={a?.loading}
      >
        {a?.loading ? (
  ...
  const renderAnalysisPanel = (rec: any, label = "Research-grade Analysis") => {
    const key = analysisKeyFor(rec);
      ...
      <div className="mt-3 rounded-lg border border-secondary/30 bg-background/70 overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 border-b border-border/20 bg-secondary/5">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              size="sm" variant="ghost" className="h-6 px-2 text-xs"
              onClick={() => streamOrderAnalysis(rec, true)}
              disabled={a.loading}
            >
